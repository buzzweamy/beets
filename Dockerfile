...
RUN \
  curl -L http://github.com/YetAnotherNerd/whatlastgenre/archive/master.zip -o /app/master.zip && \
  unzip /app/master.zip -d /app/ && \
  rm /app/master.zip

RUN \
  echo "**** install beet-install ****" && \
  pip3 install -e /beet && \
  pip3 install -e /app/whatlastgenre-master && \
